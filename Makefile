...
node_modules: yarn.lock package.json ## Install yarn dependency
	$(YARN) install
...
db-dump: .env.local vendor ## Dump migration diff
	$(CONSOLE) doctrine:schema:update --dump-sql

db-diff: .env.local vendor ## Generate a new doctrine migration
	$(CONSOLE) doctrine:migrations:diff

db-mig: .env.local vendor ## Apply doctrine migrations
	$(CONSOLE) doctrine:migrations:migrate --no-interaction --allow-no-migration

db-validate: .env.local vendor ## Validate the doctrine ORM mapping
